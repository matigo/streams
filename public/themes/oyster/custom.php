...
class Oyster {
    ...
    private function _getResourceFile() {
        ...
        $ReqPage = 'page-' . NoNull($this->settings['PgRoot'], 'gallery') . '.html';
     ...
     ** ********************************************************************* */
    private function _getGalleryHTML() {
        $count = nullInt($this->settings['count']);
        ...
        $sqlStr = readResource(SQL_DIR . '/posts/getPhotoGallery.sql', $ReplStr);
        $rslt = doSQLQuery($sqlStr);
        if ( is_array($rslt) ) {
            $SiteUrl = NoNull($this->settings['HomeURL']);
            $cdnUrl = getCdnUrl();
            $cdnUrl = 'https://matigo.ca/files';
            $html = '';

            foreach ( $rslt as $Row ) {
                $imgUrl = $cdnUrl . NoNull($Row['location']) . NoNull($Row['local_name']);
                if ( YNBool($Row['has_medium']) ) {
                    $ext = getFileExtension($imgUrl);
                    $imgUrl = str_replace(".$ext", "_medium.$ext", $imgUrl);
                }

                /* Do we need an additional CSS Class? */
                $height = nullInt($Row['image_height']);
                $width = nullInt($Row['image_width']);
                $iso = nullInt($Row['image_iso']);

                $cls = NoNull($Row['view_class']);
                if ( $cls == '' ) {
                    if ( $width > 450 && $width > ($height * 1.75) ) { $cls = 'half'; }
                    if ( $width > 650 && $width > ($height * 3.25) ) { $cls = 'full'; }
                }
                if ( $cls != '' ) { $cls = ' ' . $cls; }

                /* Construct a data string */
                $attr = '';
                if ( $height > 0 ) { $attr .= ' data-height="' . $height . '"'; }
                if ( $width > 0 ) { $attr .= ' data-width="' . $width . '"'; }
                if ( $iso > 0 ) { $attr .= ' data-iso="' . $iso . '"'; }

                /* Append the HTML */
                if ( $html != '' ) { $html .= "\r\n"; }
                $html .= tabSpace(4) . "<div class=\"grid-item" . $cls . "\"" . $attr . ">\r\n" .
                         tabSpace(5) . "<img src=\"" . $imgUrl . "\" />\r\n" .
                         tabSpace(5) . "<p>" . strip_tags(NoNull($Row['caption'], '&nbsp;')) . "</p>\r\n" .
                         tabSpace(5) . "<button><i class=\"fas fa-arrows-alt-h\"></i></button>\r\n" .
                         tabSpace(5) . "<button><i class=\"fas fa-eye-slash\"></i></button>\r\n" .
                         tabSpace(5) . "<button><i class=\"fas fa-info-circle\"></i></button>\r\n" .
                         tabSpace(4) . "</div>";
            }
        ...
        /* If we're here, there are no images to show */
        return '';
    }
