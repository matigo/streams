 ...
 *
 * Class contains the rules and methods called for the Admin site template
 */
...
class Admin {
    ...
    private function _getPageHTML( $data ) {
        ...
        $ReplStr = array( '[FONT_DIR]'      => $SiteUrl . '/fonts',
                          '[CSS_DIR]'       => $SiteUrl . '/css',
                          '[IMG_DIR]'       => $SiteUrl . '/img',
                          '[JS_DIR]'        => $SiteUrl . '/js',
        ...
        // Add the Navigation Menus
        $ReplStr['[NAV_LEFT]'] = $this->_getNavBar($data, 'left', $ReplStr);
        $ReplStr['[NAV_TOP]'] = $this->_getNavBar($data, 'top', $ReplStr);
    ...
    /**
     *  Function Returns a Completed Navigation Menu
     */
    private function _getNavBar( $data, $Nav, $ReplStr ) {
        $ThemeLocation = THEME_DIR . '/' . $data['location'];
        $ResDIR = $ThemeLocation . "/resources";

        // Which Navigation Menu Should be Returned?
        $ReqNav = 'nav-' . NoNull($Nav, 'main') . '.html';
        if ( file_exists("$ResDIR/$ReqNav") ) {
            return readResource("$ResDIR/$ReqNav", $ReplStr);
        }

        // If We're Here, the Requested Navigation Menu Does Not Exist
        return '';
    }
