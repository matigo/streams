...
class Markdown {
    ...
    public static function defaultTransform($text, $is_blurb) {
    #
    ...
    #
        ...
        if (!$parser) { $parser = new $parser_class; }

        # Transform text using parser.
        return $parser->transform($text, $is_blurb);
    }
    ...
    public function transform($text, $is_blurb) {
    #
    ...
    #
        ...
        # Mark Whether This is a Blurb or Not
        $this->is_blurb = $is_blurb;
    ...
    protected function _doAnchors_reference_callback($matches) {
        ...
        if (isset($this->urls[$link_id])) {
            ...
            $url = $this->_fixURLs($url);
            $suffix = '';
            if ( $this->is_blurb ) {
                $domain = validateURLDomain($url);
    ...
    protected function _fixURLs($url) {
        $fixes = array( 'http//'  => "http://",         'http://http://'   => 'http://',
                        'https//' => "https://",        'https://https://' => 'https://',
                        ','       => '',                'http://https://'  => 'https://',
                       );
        $url = ((stripos($clean_word, 'http') === false ) ? "http://" : '') . $url;
        return str_ireplace(array_keys($fixes), array_values($fixes), $url);
    }
    ...
    protected function _doImages_reference_callback($matches) {
        ...
        if (isset($this->urls[$link_id])) {
            $ReplStr = array( '.JPG' => '.jpg', '.PNG' => '.png', '.GIF' => '.gif', '.JPEG' => '.jpg' );
            $mimeType = '';
            $resType = 'img';
            $ext = getFileExtension($this->urls[$link_id]);
            if ( in_array($ext, array('mp3', 'mp4', 'ogg', 'flac', 'm4a')) ) {
            ...
            switch ( $resType ) {
                case 'audio':
                    $item_id = getRandomString( 12, true );
                    $result = '<audio class="audioplayer" preload="auto" controls="controls" controlsList="nodownload" data-file-id="' . strtolower($item_id) . '">' .
                                '<source type="' . $mimeType . '" src="' . $url . '">' .
                                'Your browser does not support HTML5 audio, but you can still <a href="' . $url . '" title="">download the file</a>.' .
                              '</audio>' .
                              ((NoNull($alt_text) != '') ? '<strong class="audio-title">' . $alt_text . '</strong>' : '');
                    $result = $this->hashPart($result);
                    break;
    ...
    protected function _doImages_inline_callback($matches) {
        ...
        if ( in_array($ext, array('mp3', 'ogg', 'flac', 'm4a')) ) {
            $mimeType = getMimeFromExtension($ext);
            $resType = 'audio';
        }
    ...
    protected function _doCodeBlocks_callback($matches) {
        ...
        $codeblock = str_replace(array_keys($fixes), array_values($fixes), $codeblock);
    ...
    #
        ...
        $code = str_replace(array_keys($fixes), array_values($fixes), $code);
    ...
    protected function doBlockQuotes($text) {
        $text = preg_replace_callback('/
            ...
            /xm',
            array(&$this, '_doBlockQuotes_callback'), str_replace('&gt;', '>', $text));
...
//              }
    ...
    protected function doAutoLinks($text) {
        $url_pattern = '{<((https?|ftp|dict):[^\'">\s]+)>}i';

        $text = preg_replace_callback($url_pattern, array(&$this, '_doAutoLinks_url_callback'), $text);
