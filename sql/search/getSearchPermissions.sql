SELECT ch.`id` as `channel_id`, ch.`type` as `channel_type`, ch.`privacy_type`,
       MAX(CASE WHEN ch.`type` = 'channel.site' AND ch.`privacy_type` = 'visibility.public' THEN 'Y'
                WHEN ch.`privacy_type` = 'visibility.password' AND '[SITE_TOKEN]' <> ''
                     THEN CASE WHEN '[SITE_TOKEN]' IN (SELECT SHA2(CONCAT(zsi.`guid`, '.', UNIX_TIMESTAMP(zsu.`updated_at`), '.', DATE_FORMAT(DATE_SUB(Now(), INTERVAL cnt.`num` HOUR), '%Y-%m-%d %H:00:00')), 256) as `hash`
                                                         FROM `Site` zsi INNER JOIN `SiteUrl` zsu ON zsi.`id` = szu.`site_id`
                                                                         INNER JOIN (SELECT 0 as `num` UNION ALL SELECT  1 as `num` UNION ALL SELECT  2 as `num`) cnt ON `num` >= 0
                                                        WHERE zsu.`is_deleted` = 'N' and zsu.`is_active` = 'Y' and zsi.`id` = ch.`site_id`) THEN 'Y' ELSE 'N' END
