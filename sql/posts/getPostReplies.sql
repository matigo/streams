SELECT DISTINCT po.`publish_at`, po.`thread_id`, po.`parent_id`,
       ...
       ROW_NUMBER() OVER (PARTITION BY po.`thread_id` ORDER BY po.`publish_at`, po.`id`) AS `post_num`,
       ...
       po.`id` as `post_id`, po.`guid` as `post_guid`, po.`title`, po.`value`, url.`site_url`, po.`canonical_url`, po.`reply_to`,
  ...
  FROM `Channel` ch INNER JOIN `Post` po ON ch.`id` = po.`channel_id` AND po.`is_deleted` = 'N'
                    ...
                    INNER JOIN (SELECT ch.`id` as `channel_id`, CONCAT(CASE WHEN si.`https` = 'Y' THEN 'https' ELSE 'http' END, '://', su.`url`) as `site_url`, su.`is_active`
                                 ...
                                 WHERE ch.`is_deleted` = 'N' and su.`is_active` = 'Y') url ON ch.`id` = url.`channel_id`
                    INNER JOIN (SELECT z.`id` as `post_id` FROM `Post` z 
                                 WHERE z.`is_deleted` = 'N' and z.`guid` = '[POST_GUID]') src ON src.`post_id` IN (po.`thread_id`, po.`parent_id`)
