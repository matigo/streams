...
BEGIN
    ...
    SELECT po.`id` as `post_id`, po.`parent_id`, po.`guid` as `post_guid`, po.`type` as `post_type`, po.`privacy_type`,
           ...
           pa.`guid` as `persona_guid`, pa.`is_active` as `persona_active`,
           pa.`created_at` as `persona_created_at`, ROUND(UNIX_TIMESTAMP(pa.`created_at`)) as `persona_created_unix`,
           pa.`updated_at` as `persona_updated_at`, ROUND(UNIX_TIMESTAMP(pa.`updated_at`)) as `persona_updated_unix`,
           po.`title`, po.`value`,
           ...
           ch.`created_at` as `channel_created_at`, ROUND(UNIX_TIMESTAMP(ch.`created_at`)) as `channel_created_unix`,
           ch.`updated_at` as `channel_updated_at`, ROUND(UNIX_TIMESTAMP(ch.`updated_at`)) as `channel_updated_unix`,
           ...
           si.`guid` as `site_guid`,
           si.`created_at` as `site_created_at`, ROUND(UNIX_TIMESTAMP(si.`created_at`)) as `site_created_unix`,
           si.`updated_at` as `site_updated_at`, ROUND(UNIX_TIMESTAMP(si.`updated_at`)) as `site_updated_unix`,
           ...
           po.`created_at`, ROUND(UNIX_TIMESTAMP(po.`created_at`)) as `created_unix`, po.`created_by`,
